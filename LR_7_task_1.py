...
class AntColony(object):
    ...
    def gen_path(self, start):
        ...
        for _ in range(len(self.distances) - 1):
            move = self.pick_move(self.pheromone[start], self.distances[start], visited)
            path.append((start, move))
            visited.add(move)
            start = move

        path.append((start, path[0][0]))  # повернення на початок
        return path
...
if __name__ == '__main__':
    ...
    result = ant_colony.run(start=6)   # варіант 6
